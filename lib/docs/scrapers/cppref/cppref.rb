module Docs
  class Cppref < UrlScraper
    ...
    def get_latest_version(opts)
      title = self.base_url.path.delete_prefix('/') + self.root_path
      json = fetch_json("https://en.cppreference.com/api.php?action=query&prop=revisions&titles=#{title}&rvprop=timestamp&format=json", opts)
